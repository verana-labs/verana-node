/**
 * Journey: Increase Active Governance Framework Version
 *
 * This script demonstrates how to increase the active governance framework
 * version of a Trust Registry using the TypeScript client and the generated
 * protobuf types.
 *
 * Usage:
 *   TR_ID=1 npm run test:increase-gf-version
 *   # Or let it create a TR first, then increase version
 *   npm run test:increase-gf-version
 *
...
import { MsgCreateTrustRegistry, MsgIncreaseActiveGovernanceFrameworkVersion } from "../../../src/codec/verana/tr/v1/tx";
...
async function main() {
  ...
  console.log("Journey: Increase Active Governance Framework Version (TypeScript Client)");
  console.log("=".repeat(60));
  console.log();
  ...
  if (process.env.TR_ID) {
    ...
    const createFee = await calculateFeeWithSimulation(
      ...
      account.address,
      [createMsg],
      "Creating Trust Registry for increase version test"
    );
    ...
    if (createResult.code !== 0) {
      console.log("  ❌ Failed to create Trust Registry for increase version test");
  ...
  // Step 5: Add a document for version 2 first (required before increasing version)
  console.log("Step 5: Adding Governance Framework Document for version 2 (required before increasing version)...");
  const addDocMsg = {
    ...
    value: MsgAddGovernanceFrameworkDocument.fromPartial({
      ...
      docDigestSri: "sha384-NewDocumentHash123456789012345678901234567890123456789012345678901234567890",
      version: 2, // Must be 2 or higher (version 1 already exists from TR creation)
    }),
  };

  const addDocFee = await calculateFeeWithSimulation(
    client,
    account.address,
    [addDocMsg],
    "Adding document for version 2"
  );

  const addDocResult = await client.signAndBroadcast(
    account.address,
    [addDocMsg],
    addDocFee,
    "Adding document for version 2"
  ...
  if (addDocResult.code !== 0) {
    console.log("  ❌ Failed to add document for version 2");
    console.log(`  Error: ${addDocResult.rawLog}`);
    process.exit(1);
  }
  console.log("  ✓ Added document for version 2");
  console.log();

  // Step 6: Increase Active Governance Framework Version
  console.log("Step 6: Increasing Active Governance Framework Version transaction...");
  const msg = {
  ...
  try {
    const fee = await calculateFeeWithSimulation(
      ...
      account.address,
      [msg],
      "Increasing Active Governance Framework Version via TypeScript client"
    );
    ...
    console.log();
    if (result.code === 0) {
      console.log("✅ SUCCESS! Active Governance Framework Version increased successfully!");
      console.log("=".repeat(60));
