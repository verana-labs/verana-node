 ...
 * Journey: Remove DID
 *
 * This script demonstrates how to remove a DID from the DID Directory using the
 * TypeScript client and the generated protobuf types.
 *
 * Usage:
 *   DID="did:verana:example" npm run test:remove-did
 *   # Or let it add a DID first, then remove it
 *   npm run test:remove-did
 *
...
async function main() {
  ...
  console.log("Journey: Remove DID (TypeScript Client)");
  console.log("=".repeat(60));
  console.log();
  ...
  if (process.env.DID) {
    ...
    if (addResult.code !== 0) {
      console.log("  ❌ Failed to add DID for remove test");
  ...
  // Step 5: Remove DID message
  console.log("Step 5: Creating Remove DID transaction...");
  const msg = {
  ...
  try {
    const fee = await calculateFeeWithSimulation(
      ...
      account.address,
      [msg],
      "Removing DID via TypeScript client"
    );
    ...
    console.log();
    if (result.code === 0) {
      console.log("✅ SUCCESS! DID removed successfully!");
      console.log("=".repeat(60));
