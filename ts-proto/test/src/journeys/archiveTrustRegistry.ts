/**
 * Journey: Archive Trust Registry
 *
 * This script demonstrates how to archive/unarchive a Trust Registry using the
 * TypeScript client and the generated protobuf types.
 *
 * Usage:
 *   TR_ID=1 npm run test:archive-tr
 *   # Or let it create a TR first, then archive it
 *   npm run test:archive-tr
 *
...
import { MsgCreateTrustRegistry, MsgArchiveTrustRegistry } from "../../../src/codec/verana/tr/v1/tx";
...
async function main() {
  ...
  console.log("Journey: Archive Trust Registry (TypeScript Client)");
  console.log("=".repeat(60));
  console.log();
  ...
  if (process.env.TR_ID) {
    ...
    const createFee = await calculateFeeWithSimulation(
      ...
      account.address,
      [createMsg],
      "Creating Trust Registry for archive test"
    );
    ...
    if (createResult.code !== 0) {
      console.log("  ❌ Failed to create Trust Registry for archive test");
  ...
  // Step 5: Archive Trust Registry
  console.log("Step 5: Archiving Trust Registry transaction...");
  ...
  try {
    const fee = await calculateFeeWithSimulation(
      ...
      account.address,
      [msg],
      "Archiving Trust Registry via TypeScript client"
    );
    ...
    console.log();
    if (result.code === 0) {
      console.log(`✅ SUCCESS! Trust Registry ${archive ? "archived" : "unarchived"} successfully!`);
      console.log("=".repeat(60));
